...
| Token (JWT) | Digitaal ondertekend 'bewijsje' dat met een bericht wordt meegestuurd en informatie over de afzender en het doel bevat. |
...
De AVG en de Wabvpz leggen de plicht tot passende beveiliging en betrouwbare authenticatie van medewerkers en systemen bij iedere zorgaanbieder zelf, als zelfstandig verwerkingsverantwoordelijke. Die verantwoordelijkheid rust dus uitdrukkelijk op de organisatie die de zorgverlener of het systeem beheert, niet op een andere partij waarmee gegevens worden uitgewisseld. Om drie samenhangende redenen is het niet nodig, en ook niet wenselijk, dat een uitwisselingspartner deze interne authenticatie nog eens overdoet of zelfstandig controleert:

1. **Geen wettelijke opdracht.** De wet belegt de authenticatieplicht eenduidig bij de zorgaanbieder die de zorgverlener of het systeem beheert. Nergens wordt van de uitwisselingspartner gevraagd deze te herhalen of te verifiëren. Dat zou namelijk een tweede, wettelijk niet-voorziene verantwoordelijkheid creëren voor iets waarop die partner geen zeggenschap heeft.
2. **Geen toegevoegde betrouwbaarheid.** De uitwisselingspartner heeft geen toegang tot de identiteits- en autorisatievoorzieningen (IAM) van de andere zorgaanbieder. Een eigen controle zou dus hoe dan ook moeten steunen op diezelfde bron: de interne administratie van die uitwisselingspartner. Dat voegt geen zekerheid toe, maar wel complexiteit en een tweede plek waar iets fout kan gaan.
3. **Strijdig met dataminimalisatie.** Om zelf te kunnen verifiëren, zou de uitwisselingspartner toegang moeten krijgen tot persoonsgegevens van medewerkers van de andere organisatie, bijvoorbeeld een overzicht van geautoriseerde zorgverleners. Dat gaat verder dan noodzakelijk voor het doel van de uitwisseling en is daarmee in strijd met het AVG-beginsel van dataminimalisatie.

In plaats van een eigen controle toont iedere zorgaanbieder de betrouwbaarheid van zijn interne processen aantoonbaar aan via het verplichte normenkader van NEN 7510 (informatiebeveiliging) en NEN 7512 (veilige verbindingen tussen organisaties). De uitwisselingspartner mag op die aantoonbare naleving vertrouwen, zonder dit zelf te hoeven controleren. Dit vertrouwen-op-normen in plaats van vertrouwen-op-eigen-controle is het uitgangspunt dat in hoofdstuk 4 wordt uitgewerkt tot het federatieve vertrouwensmodel.

Voor de betrouwbaarheid van het inlogmiddel waarmee de individuele zorgverlener zich binnen dit interne proces identificeert, vormt de eIDAS-verordening het Europese referentiekader: deze definieert betrouwbaarheidsniveaus (laag, substantieel, hoog) voor inlogmiddelen van personen. Op dit moment is niveau hoog vereist voor de verwerking van de BgZ en eOverdracht (zie paragraaf 7.2). Het eIDAS-niveau is een aanvulling op de NEN 7510-eisen aan het onderliggende IAM-proces, niet een vervanging ervan. Ook dit blijft, net als de authenticatie zelf, een interne aangelegenheid: de uitwisselingspartner controleert niet welk eIDAS-niveau daadwerkelijk is gebruikt, maar vertrouwt erop dat de zorgaanbieder het afgesproken niveau hanteert. Dit onderscheid is van belang omdat eIDAS uitdrukkelijk niet van toepassing is op de authenticatie van de zorgaanbieder als organisatie (zie paragraaf 3.2).
...
Dit hoofdstuk legt het ene ontwerpprincipe uit waarop alle antwoorden in deze notitie zijn gebaseerd. 

### 4.1 Het probleem: wie vertrouw je, en op basis waarvan?

Als zorgaanbieder A gegevens deelt met zorgaanbieder B, moet A ergens op kunnen vertrouwen voordat hij gegevens vrijgeeft. Er zijn grofweg twee manieren om dat vertrouwen in te vullen:

1. **A controleert zelf tot op het niveau van de individuele medewerker.** A wil weten: welke zorgverlener van B vraagt dit op, met welk inlogmiddel, en is die persoon daadwerkelijk bevoegd? Dat vereist dat A toegang heeft tot (delen van) het personeels- en inlogsysteem van B.
2. **A controleert alleen de organisatie, en vertrouwt op het interne proces van B.** A stelt vast dát het verzoek daadwerkelijk van organisatie B komt, en gaat ervan uit dat B zijn eigen medewerkers al betrouwbaar heeft geauthenticeerd en geautoriseerd.

De eerste manier lijkt op het eerste gezicht veiliger, maar is in de praktijk niet haalbaar. Zorgaanbieders hebben hun eigen manier van authenticatie en autorisatie. En ook de rollen en het toekennen van deze rollen aan zorgmedewerkers en systemen is niet altijd gelijk. Daarom is het matchen van deze gegevens tussen duizenden zorgaanbieders onbegonnen werk. VWS kiest daarom voor de tweede manier: het **federatieve vertrouwensmodel**.
...
Omdat de uitwisselingspartner geen controle op persoons- of systeemniveau uitvoert, volstaat het dat elke transactie de context meedraagt die nodig is voor autorisatie, logging en herleidbaarheid op organisatieniveau. Dat zijn drie attributen: de organisatie-identiteit (URA), het doel van de opvraging (bijvoorbeeld 'behandeling') en het organisatietype (nodig voor de toestemmingscontrole).

Deze attributen worden meegegeven als verklaring van de zorgaanbieder, niet als te verifiëren bewijs. De partij die de gegevens beschikbaar stelt, gebruikt ze uitsluitend voor het autorisatiebesluit en legt ze vast in de logging. Zij valideert niet zelf de onderliggende authenticatie. Doordat BgZ en eOverdracht dezelfde attributenset en hetzelfde verklaringsformaat hanteren, is het autorisatie- en loggingsproces bij de bron voor beide uitwisselingen identiek.

### 5.2 Transportniveau: wat de ontvanger controleert

De ontvangende partij controleert uitsluitend dat de verklaring afkomstig is van een geauthenticeerde organisatie, niet de juistheid van de inhoud. Hoe een zorgaanbieder het interne inloggen technisch invult (wachtwoord met MFA, smartcard, UZI-pas, single sign-on vanuit het EPD/ECD) is een lokale keuze binnen de normkaders en is voor de uitwisselingspartner niet zichtbaar en niet relevant. Dit maakt de aanpak direct toepasbaar in de BgZ en eOverdracht. De verwachting is dat een toekomstige migratie richting Dezi hier geen verandering in brengt.
...
Het federatieve vertrouwensmodel, zoals besproken in hoofdstuk 4, rust op één expliciete ontwerpbeslissing: een zorgaanbieder verifieert bij uitwisseling niets op het niveau van de individuele zorgverlener of het individuele systeem van de uitwisselingspartner, maar vertrouwt op diens interne processen. De dossierhouder, op wie de geheimhoudingsplicht rust, deelt gegevens met de opvragende partij en ontleent zijn zekerheid dus aan de authenticatie en autorisatie die daar hebben plaatsgevonden.

Het kernrisico volgt direct uit die beslissing: een tekortschietend intern proces bij één zorgaanbieder werkt door in de vertrouwelijkheid van dossiers bij alle uitwisselingspartners, zonder dat die uitwisselingspartners dit in de transactie kunnen detecteren. De borging moet daarom volledig buiten de transactie worden georganiseerd, op drie momenten:
...
Iedere zorgaanbieder is en blijft zelfstandig verwerkingsverantwoordelijke (AVG) voor de verwerking in de eigen systemen, inclusief het knooppunt of de intermediair die namens hem optreedt, en is het aanspreekpunt voor patiënten bij de uitoefening van de privacyrechten van de patiënt. Het federatieve model verschuift die verantwoordelijkheid niet, maar maakt haar scherp belegbaar: omdat de uitwisselingspartner uitsluitend afgaat op verklaringen van de zorgaanbieder, is die zorgaanbieder juridisch volledig aansprakelijk voor de juistheid daarvan.
...
Het organisatorisch spoor borgt dat een zorgaanbieder de processen waarop anderen vertrouwen daadwerkelijk op orde heeft en houdt, langs dezelfde drie momenten als in paragraaf 7.1.
...
Geen van deze extensies dekt de gevraagde attributenset volledig: zowel het UDAP B2B- als het IHE IUA JWT-extensieobject bevat wél het doel (purpose_of_use), maar géén attribuut voor het organisatietype. Het meedragen van het organisatietype vergt dus hoe dan ook een eigen aanvulling.
...
sequenceDiagram
    autonumber
    participant A as Zorgaanbieder A (client)
    participant AS as Authorization Server (zorgaanbieder B)
    participant RS as Resource Server FHIR (zorgaanbieder B)
...
Zowel de client-assertion in het tokenverzoek als het access token worden geborgd met de sleutel behorend bij het UZI-certificaat van de zorgaanbieder; de ontvanger valideert de handtekening via het JWKS-endpoint van de afgevende organisatie. Daarmee is alleen de organisatie-identiteit (URA) cryptografisch verifieerbaar; organisatietype en doel reizen mee als verklaring van de zorgaanbieder in het RAR-object, precies zoals hoofdstuk 5 voorschrijft.
...
- **NL.GISA.AUTHN.ZA.6:** De ontvangende partij MOET van ieder inkomend token de handtekening valideren via het JWKS-endpoint van de afgevende organisatie, alvorens de transactie te verwerken.
- **NL.GISA.AUTHN.ZA.7:** De zorgaanbieder MAG een knooppunt of intermediair inzetten als technisch vertegenwoordiger. De oorspronkelijke organisatie-identiteit (URA) MOET daarbij end-to-end worden meegevoerd en MAG NIET worden vervangen door de identiteit van een eventuele intermediair.
...
- **NL.GISA.AUTHN.ZV.2:** Een zorgaanbieder MAG NIET zelf authenticatie of verificatie uitvoeren op het niveau van individuele zorgverleners of individuele systemen van de uitwisselingspartner. Zij MOET erop vertrouwen dat de uitwisselingspartner deze authenticatie zelf, binnen het eigen beveiligingsdomein, betrouwbaar heeft uitgevoerd (zie paragraaf 3.1 en hoofdstuk 4).
...
- **NL.GISA.AUTHZ.ZA.1:** De beschikbaarstellende partij MOET bij iedere opvraging een autorisatiecontrole uitvoeren op basis van het meegeleverde organisatietype en doel, conform het doelgebonden autorisatieprotocol.
- **NL.GISA.AUTHZ.ZA.2:** De ontvangende partij MOET uitsluitend de herkomst en integriteit van de verklaring valideren.
